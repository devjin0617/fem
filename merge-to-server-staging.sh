...
# server/staging 브랜치로 체크아웃
echo "server/staging 브랜치로 이동 중..."
git checkout server/staging

if [ $? -ne 0 ]; then
    echo "❌ server/staging 브랜치로 체크아웃 실패"
    exit 1
fi

# server/staging 브랜치 최신화 (선택사항)
echo "server/staging 브랜치 pull 중..."
git pull origin server/staging

# 이전 브랜치를 server/staging에 머지
echo "$CURRENT_BRANCH 브랜치를 server/staging에 머지 중..."
git merge $CURRENT_BRANCH --no-edit -m "Merge branch '$CURRENT_BRANCH' into server/staging"
